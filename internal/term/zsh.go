...
func zshHookLines(commandName string) []string {
	return []string{
		...
		fmt.Sprintf(
			`  assignments="$(command %s record start --format shell --session-id "$RICHHISTORY_SESSION_ID" --session-name "${RICHHISTORY_SESSION_NAME:-}" --seq "$RICHHISTORY_SESSION_SEQ" --shell zsh --shell-pid "$$" --tty "$tty_name" --pwd "$PWD" --command "$1" --started-at "$started_at")" || return`,
			commandName,
		),
		`  eval "$assignments"`,
